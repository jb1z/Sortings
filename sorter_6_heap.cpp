...
void heapsorter::sort(QVector<int> arrToSort, QVector <QPushButton*>& arrButtons){
    int *buff = new int[arrToSort.size()+1];
    for(int i = 0; i < arrToSort.size();++i){
        this->insert(buff,i,arrToSort[i]);
    }
    for(int i = 0; i < arrToSort.size();i++){
        arrToSort[arrToSort.size() - 1 - i] = buff[1];
        this->pop(buff,arrToSort.size() - i);
    }
    delete []buff;
}
